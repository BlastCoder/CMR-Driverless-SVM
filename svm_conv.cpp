...
#include "cones.h"
// TODO: CONVERT FROM STD::NUMBERS TO CMATH FOR PI (C++ 17 not 20) - did
// normalize when to use pointsVector and when to use vectorRow - did
// explicitily write std::vector<std::vector<double>> instead of vectorRow - did
// change pointsVector to conesList - did
/* augmentConesCircle and augmentDatset should be void 
   methods which change underlying object instead of returning */ 
// free prob and param at end instead of current spot (svm model uses pointers to prob) - did
...
*/ 
// manan: check if for either col and row, where the value changes from 1 to 0 or vice versa, and thats a boundary point
...
conesList cones_to_midline(Cones cones) {
    ...
    node1[1].value = 2.2;
    node1[2].index = -1;
    double pred = svm_predict(model, node1);
    std::cout << "Prediction 1: " << pred << std::endl;
    delete[] node1;
    ...
    node2[1].value = 3.3;
    node2[2].index = -1;
    double pred2 = svm_predict(model, node2);
    std::cout << "Prediction 2: " << pred << std::endl;
    delete[] node2;
...
int main() {
    ...
    std::vector<std::vector<double>> blue1 = {
        {0.0, 2.0},
        {2.0, 0.0},
        {0.0, -2.0},
        {-2.0, 0.0}
    ...
    std::vector<std::vector<double>> yellow1 = {
        {0.0, 3.0},
        {3.0, 0.0},
        {0.0, -3.0},
        {-3.0, 0.0}
    ...
    */
    ...
    Cones conesStraight;
    conesStraight.addMultipleBlue(blue_straight);
    conesStraight.addMultipleYellow(yellow_straight);
    std::vector<std::pair<double, double>> straightAns = cones_to_midline(conesStraight);

    std::cout << "straight midline: " << std::endl;

    for (const auto& p : straightAns) {
        std::cout << "(" << p.first << ", " << p.second << ")," << std::endl;
    }
